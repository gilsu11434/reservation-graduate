...
- `my-reservation.html`: 내 예약 기간, 날짜별 안전수칙·당일 퇴실, 이용확인서
...
- `supabase-remove-certificate-requirement.sql`: 수료증 제출·인증 절차 제거 및 기존 예약 상태 전환
...
2. `SQL Editor > New query`에서 아래 파일을 한 번에 하나씩 순서대로 실행합니다.
   ...
   15. `supabase-daily-checkout.sql`
...
3. 신규 프로젝트에서는 `supabase-fix-usage-reports-created-at.sql`과 `supabase-manual-usage-report-approval.sql`을 실행하지 않습니다.
   이전 버전에서 `supabase-graduate-date-range.sql`을 이미 실행했다면, 다른 사용자 간 중복 허용·본인 기간 중복 제한과 최신 호실 목록 적용을 위해 최신 파일을 다시 한 번 실행합니다.
...
메인 페이지의 `이용확인서 양식 다운로드` 버튼을 누르면 `forms/학부실험실_이용확인서.hwp`를 내려받을 수 있습니다.
한글에서 내용을 작성한 다음 PDF·JPG·PNG 형식으로 저장하고, `내 예약` 페이지에서 제출하세요.

메인 페이지의 `예약 매뉴얼 다운로드` 버튼을 누르면 `forms/학부실험실_예약매뉴얼.pdf`를 내려받을 수 있습니다.

## 승인 진행 방식

1. 다른 사용자끼리는 호실이나 날짜가 겹치는 예약을 신청할 수 있지만, 한 사용자는 호실에 관계없이 같은 기간에 하나의 예약만 신청할 수 있습니다. 예약 상태는 `승인 대기`가 됩니다.
2. 관리자가 달력의 예약 건을 눌러 예약을 승인하거나 거절하며, 상세화면에서 시작 날짜와 종료 날짜를 변경할 수 있습니다. 날짜 변경 시에도 같은 예약자의 다른 예약 기간과는 겹칠 수 없습니다.
