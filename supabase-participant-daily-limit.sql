-- 팀이 아닌 참여자 개인별로 하루 이용시간을 최대 2시간으로 제한합니다.
-- 한국시간 기준 00:00부터 다음 날 00:00까지 계산합니다.
...
-- 예약 생성 전에 각 참여자의 당일 누적시간을 확인하는 함수입니다.
create or replace function public.check_participant_daily_hours(
  p_emails text[],
...
as $$
      ...
      coalesce(
        (
          ...
          where lower(trim(member.member_email)) = supplied.member_email
            ...
            and reservation.start_at >= parameters.day_start
            and reservation.start_at < parameters.day_end
        ),
        0
  ...
  select
    ...
    calculated.used_hours + calculated.requested_hours <= 2
      as is_allowed
  from calculated_usage as calculated
  order by calculated.member_email;
$$;

revoke all on function public.check_participant_daily_hours(
  text[],
  timestamptz,
  timestamptz
) from public;

grant execute on function public.check_participant_daily_hours(
  text[],
  timestamptz,
  timestamptz
) to authenticated;

-- 브라우저 검사를 우회해 참여자를 저장해도 하루 2시간을 넘지 못하게 합니다.
create or replace function public.enforce_participant_daily_limit()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  ...
  where lower(trim(member.member_email)) = new.member_email
    ...
    and reservation.status::text <> 'cancelled'
    and reservation.start_at >= v_day_start
    and reservation.start_at < v_day_end;

  if v_used_hours + v_requested_hours > 2 then
    raise exception
      '참여자 %의 일일 이용시간이 2시간을 초과합니다. 기존 %시간 + 신청 %시간',
      new.member_email,
...
create trigger trigger_enforce_participant_daily_limit
before insert or update of member_email, reservation_id
on public.reservation_members
for each row
execute function public.enforce_participant_daily_limit();
